...
class UploadClient {
  ...
  Future<UploadImageResponseBody> uploadImageForAnalysis(
    ...
    // 仮実装: サンプルデータを返す
    await Future<void>.delayed(const Duration(seconds: 2)); // API呼び出しをシミュレート
    ...
    return const UploadImageResponseBody(
      ...
      goodPoints: [
        GoodPoint(
          title: '高品質タンパク源',
          content: '新鮮なチキンを贅沢に使用しており、筋肉維持に欠かせないアミノ酸スコアが高いです。',
        ),
        GoodPoint(
          title: 'クリーンな原材料',
          content: '人工保存料や合成着色料が一切含まれておらず、内臓への負担が少ない安心設計です。',
        ),
      ],
      badPoints: [
        BadPoint(
          title: '炭水化物の含有量',
          content: '炭水化物がやや多めで、運動量の少ない室内猫には体重管理に注意が必要です。',
        ),
      ],
